...
- **`--algorithms` parameter**: Not yet implemented. Currently hardcoded to `FP-Het` since only one scheduler exists. When EDF/LLF schedulers are added, extend `SweepRow.algorithm` and add `--algorithms` flag.
